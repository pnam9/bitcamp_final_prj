...
<div class="container-fluid px-4">
	<h1 class="mt-4">결제목록관리</h1>
		<div class="card mb-4">
			<div class="card-header">
			<i class="fas fa-table me-1"></i>결제목록
			 	<a style="float:right" href="<c:url value='incomeexcelDown.do'/>">
                            ...
                            <c:forEach items="${allincomelist}" var="in" varStatus="i">
                           	<tbody>
                            	<tr>
                                   ...
                                   <c:choose>
									    <c:when test="${in.canum eq '1'}">
										정육
									    </c:when>
									    <c:when test="${in.canum eq '2'}">
										수산
									    </c:when>
									    <c:when test="${in.canum eq '3'}">
										채소
									    </c:when>
									    <c:when test="${in.canum eq '4'}">
										과일
									    </c:when>
									    <c:when test="${in.canum eq '5'}">
										밀키트
									    </c:when>
									    <c:otherwise>
									    미표기
                                   ...
                                   <td style="text-align: center;">${in.itemnum}</td>
                                   <td style="text-align: center;">${in.ordernum}</td>
                                   <td style="text-align: center;">${in.title}</td>
                                   <td style="text-align: center;">${in.count}</td>
                                   <td style="text-align: center;">${in.price}</td>
                                   <td style="text-align: center;">${in.pdate.substring(2,11)}</td>
                               </tr>
                            </c:forEach>
                            </tbody>
				</table>
...
$("#pagination").twbsPagination({
	...
	onPageClick:function(event, page){
		location.href = "allincomelist.do?search=" + $("#_search").val() + "&choice=" + $("#_choice").val() + "&pageNumber=" + (page - 1);	
	}
});	


$("#btnSearch").click(function () {
	location.href = "allincomelist.do?search=" + $("#_search").val() + "&choice=" + $("#_choice").val();	
});
